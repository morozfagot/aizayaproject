...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	constructor({
		...
		const pipelineLogDir = "C:\\Users\\Moroz\\Desktop\\AIWorkFlowContext\\morozcode-logs"
	...
	public async *attemptApiRequest(
		...
		// 2.9.15.7: Conversation History NEVER sent to model — skip manageContext entirely

		// Get the effective API history — RRR messages only
		...
		// 2.9.15.7: Conversation History NEVER sent to model — RRR only
		...
		// 2.9.15.7: RRR must be configured — no silent fallback to conversation history
		const rrrEmbedderModelId = this.providerRef.deref()?.getCodebaseIndexEmbedderModelId?.()
		if (!rrrEmbedderModelId) {
			throw new Error(
				"RRR is not configured: no embedding model ID. " +
				"Enable codebase indexing in VS Code: Roo Code → Code Indexing → choose an Embedding Model, then click 'Save & Index'. " +
				"Conversation History is NEVER sent to the model — only RRR-enriched messages are used."
			)
		}
		...
		try {
			...
			const messagesSummary = `Total: ${cleanConversationHistory.length}, user: ${userCount}, assistant: ${assistantCount}, enriched: ${enrichedContext}`
			...
			if (isContextWindowExceededError && retryAttempt < MAX_CONTEXT_WINDOW_RETRIES) {
				...
				// 2.9.15.7: handleContextWindowExceededError removed — conversation history not sent
